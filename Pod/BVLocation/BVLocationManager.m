...
- (void)initGimbal {
  if ([self.class isValidUUID:_apiKey]) {
    ...
    NSAssert(NO, @"You must provide a valid Location API Key before using "
                 @"BVLocation");
  }
}
...
- (void)receiveLocationAPIKey:(NSNotification *)notification {
  ...
  if ([[notification name] isEqualToString:LOCATION_API_KEY_SET_NOTIFICATION]) {
    [[BVLogger sharedLogger]
        verbose:@"Recieved notifcation for location configuration"];

    _apiKey = [notification.userInfo
        valueForKeyPath:LOCATION_API_KEY_SET_NOTIFICATION];
    [self initGimbal];
  }
}
...
+ (void)startLocationUpdates {
  if ([CLLocationManager authorizationStatus] ==
    ...
    NSAssert([[self sharedManager] apiKey],
             @"Unable to start updating location. BVLocation API Key not set");
    [GMBLPlaceManager startMonitoring];
    [GMBLCommunicationManager startReceivingCommunications];
    [[BVLogger sharedLogger]
        verbose:@"Successfully started updating location."];

  } else {
    [[BVLogger sharedLogger]
        verbose:@"Unable to start updating location. Insufficent permission"];
  }
}
...
- (void)placeManager:(GMBLPlaceManager *)manager
  ...
  [self callbackToDelegates:@selector(didBeginVisit:)
...
- (void)placeManager:(GMBLPlaceManager *)manager
  ...
  [self callbackToDelegates:@selector(didEndVisit:)
             withAttributes:attributeDictionary];
}
...
- (void)callbackToDelegates:(SEL)selector
  ...
  if (![clientId isEqualToString:[BVSDKManager sharedManager]
  ...
  for (DelegateContainer *container in delegates) {
    if (container.delegate) {
      ...
      [[BVLogger sharedLogger]
          verbose:[NSString stringWithFormat:@"Visit Recorded: %@",
                                             visit.description]];
...
+ (DelegateContainer *)getContainerForDelegate:
    (id<BVLocationManagerDelegate>)delegate {
  for (DelegateContainer *container in
       [[self sharedManager] registeredDelegates]) {

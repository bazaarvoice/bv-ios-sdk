...
didReceiveNotificationResponse:(UNNotificationResponse *)response
             completionHandler:
                 (void (^)(UNNotificationContentExtensionResponseOption option))
                     completion {
  NSString *Id =
  ...
  if ([[response actionIdentifier] isEqualToString:ID_REPLY] ||
    ...
    NSString *urlscheme = [NSString
        stringWithFormat:@"%@://www.bvsdk.com?type=review&subtype=%@&id=%@",
                         response.notification.request.content
                             .userInfo[USER_INFO_URL_SCHEME],
                         ([self getProductType] == ProductTypeStore)
                             ? @"store"
                             : @"product",
                         Id];
    [self.extensionContext openURL:[NSURL URLWithString:urlscheme]
                 completionHandler:nil];

    [BVNotificationsAnalyticsHelper
        queueAnalyticEventForReviewUsedFeature:ID_REPLY
                                        withId:Id
                                andProductType:[self getProductType]];
    ...
    [[UNUserNotificationCenter currentNotificationCenter]
        addNotificationRequest:response.notification.request
...
- (void)didReceiveNotification:(UNNotification *)notification {
  [[BVAnalyticEventManager sharedManager]
      setClientId:notification.request.content.userInfo[USER_INFO_CLIENTID]];
}

- (void)launchAppWithType:(NSString *)type

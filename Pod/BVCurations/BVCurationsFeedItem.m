...
- (id)initWithDict:(NSDictionary *)dict
  ...
  if (self) {
    ...
    if (dict != nil) {
      ...
      self.explicitPermissionStatus =
          [dict objectForKey:@"explicit_permission_status"];
      self.author = [[BVCurationsPostAuthor alloc]
          initWithDict:[dict objectForKey:@"author"]];
      self.links = [dict objectForKey:@"links"];
      self.coordinates = [[BVCurationsCoordinates alloc]
          initWithDict:[dict objectForKey:@"coordinates"]];
      ...
      // If we have tags and referenced products we can build the product
      // dictionary model
      ...
      if (referencedProducts != nil && self.tags != nil &&
        ...
        for (NSString *tag in self.tags) {
          ...
          BVCurationsProductDetail *item =
              [referencedProducts objectForKey:tag];
          if (item) {
...
- (NSString *)description {
  ...
  return
      [NSString stringWithFormat:@"Curations Feed Item: Channel: %@ - Text: %@",
...
- (NSString *)description {
  ...
  return [NSString
      stringWithFormat:@"Locations: Lat:%@ / Long:%@",
                       [NSString stringWithFormat:@"%@", self.latitude],
                       [NSString stringWithFormat:@"%@", self.latitude]];
}
...
- (id)initWithDict:(NSDictionary *)dict {
  ...
  if (self) {
    ...
    if (dict != nil) {
      ...
      SET_IF_NOT_NULL(self.imageServiceUrl,
                      [dict objectForKey:@"image_service_url"])
...
- (NSString *)description {
  ...
  return [NSString stringWithFormat:@"Photo: Origin: %@, URL: %@", self.origin,
...
- (NSString *)description {
  ...
  return [NSString stringWithFormat:@"Photo: Usename: %@, Channel: %@",
...
- (id)initWithDict:(NSDictionary *)dict withKey:(NSString *)key {
  ...
  if (self) {
    ...
    if (dict != nil) {
      ...
      SET_IF_NOT_NULL(self.productPageUrl,
                      [dict objectForKey:@"ProductPageUrl"])
      SET_IF_NOT_NULL(self.productDescription,
...
- (NSString *)description {
  ...
  return [NSString
      stringWithFormat:@"Key: %@, ID: %@ Title: %@, Rating: %.02f(%ld)",
                       self.productKey, self.productId, self.productName,
                       [self.avgRating floatValue],
                       (long)[self.totalReviewCount integerValue]];
}

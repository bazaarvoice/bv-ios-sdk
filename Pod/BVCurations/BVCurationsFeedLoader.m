    ...
    NSURLSessionDataTask *downloadTask = [[NSURLSession sharedSession]
        ...
        if ((!error && urlResp.statusCode < 300) && data != nil){
            ...
            if (!errorJSON){
                ...
                [[BVLogger sharedLogger] verbose:[NSString stringWithFormat:@"RESPONSE: %@", responseDict]];
                
                 // check response body status code first. Curations API will return a 200 response on failures, but put the HTTP status in the "code" value.
                int status = 200;
                ...
                if (status < 300){
                    ...
                    NSMutableDictionary *referencedProdcts = [NSMutableDictionary dictionary];
                    
                    // Check and see if product data is present
                    NSDictionary *productData = [responseDict objectForKey:@"productData"];
                    ...
                    if (productData != nil){
                        ...
                        for (NSString *key in productData.allKeys){
                            ...
                            BVCurationsProductDetail * prod = [[BVCurationsProductDetail alloc] initWithDict:dictToParse withKey:key];
                    ...
                    for(NSDictionary* update in updates) {
                        BVCurationsFeedItem * feedItem = [self getFeedItem:update withReferencedProducts:referencedProdcts withExternalId:feedRequest.externalId];
...
-(BVCurationsFeedItem *)getFeedItem:(NSDictionary*)update withReferencedProducts:(NSDictionary *)referencedProducts withExternalId:(NSString *)externalId {
    
    NSDictionary* data = [update objectForKey:@"data"];
    return [[BVCurationsFeedItem alloc] initWithDict:data withReferencedProducts:referencedProducts withExternalId:externalId];

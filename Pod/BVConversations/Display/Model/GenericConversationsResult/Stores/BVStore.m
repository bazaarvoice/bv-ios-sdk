...
-(id _Nonnull)initWithApiResponse:(NSDictionary* _Nonnull)apiResponse{
    ...
    if(self) {
        ...
        if (self.attributes){
            self.storeLocation = [[BVStoreLocation alloc] initWithStoreAtrributes:self.attributes];
        }
...
- (CLLocation * _Nullable)getCLLocation {
    ...
    if (storeLongitude != 0.0 && storeLatitute != 0.0){
        location = [[CLLocation alloc] initWithLatitude:storeLatitute longitude:storeLongitude];
    }
...
- (BOOL)hasGeoLoation{
    ...
    if (self.storeLocation != nil && self.storeLocation.longitude != nil && self.storeLocation.latitude != nil){
        hasLocation = YES;
    }

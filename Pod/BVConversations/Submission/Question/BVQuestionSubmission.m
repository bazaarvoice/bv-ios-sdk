...
- (void)submit:(nonnull QuestionSubmissionCompletion)success
  ...
  if (self.action == BVSubmissionActionPreview) {
    [[BVLogger sharedLogger]
        warning:@"Submitting a 'BVQuestionSubmission' with action set to "
                @"`BVSubmissionActionPreview` will not actially submit "
                @"the question! Set to `BVSubmissionActionSubmit` for "
                @"real submission."];
    [self submitPreview:success failure:failure];
  } else {
    [self submitPreview:^(BVQuestionSubmissionResponse *__nonnull response) {
...
- (void)submitPreview:(QuestionSubmissionCompletion)success
...
- (void)submitForReal:(QuestionSubmissionCompletion)success
              failure:(ConversationsFailureHandler)failure {
  ...
  for (BVUploadablePhoto *photo in self.photos) {
    [photo uploadForContentType:BVPhotoContentTypeQuestion
...
- (void)
submitQuestionWithPhotoUrls:(BVSubmissionAction)action
                  photoUrls:(nonnull NSArray<NSString *> *)photoUrls
              photoCaptions:(nonnull NSArray<NSString *> *)photoCaptions
                    success:(nonnull QuestionSubmissionCompletion)success
                    failure:(nonnull ConversationsFailureHandler)failure {
  ...
  NSString *urlString =
      [NSString stringWithFormat:@"%@submitquestion.json",
      ...
      dataTaskWithRequest:request
        completionHandler:^(NSData *data, NSURLResponse *response,
          ...
          @try {
            ...
            BVQuestionSubmissionErrorResponse *errorResponse =
                [[BVQuestionSubmissionErrorResponse alloc]
            ...
            if (httpError) {
              ...
              // Fire event now that we've confirmed the question was
              // successfully uploaded.
              BVFeatureUsedEvent *writeQuestionEvent = [
              ...
              BVQuestionSubmissionResponse *response =
                  [[BVQuestionSubmissionResponse alloc]
                      initWithApiResponse:json];

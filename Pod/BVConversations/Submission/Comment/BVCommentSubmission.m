...
- (void)submit:(nonnull CommentSubmissionCompletion)success
  ...
  if (self.action == BVSubmissionActionPreview) {
    [[BVLogger sharedLogger]
        warning:@"Submitting a 'BVCommentSubmission' with action set to "
                @"`BVSubmissionActionPreview` will not actially submit "
                @"the comment! Set to `BVSubmissionActionSubmit` for real "
                @"submission."];
    [self submitPreview:success failure:failure];
  } else {
    [self submitPreview:^(BVCommentSubmissionResponse *__nonnull response) {
...
- (void)submitPreview:(CommentSubmissionCompletion)success
...
- (void)submitForReal:(CommentSubmissionCompletion)success
              failure:(ConversationsFailureHandler)failure {
  ...
  for (BVUploadablePhoto *photo in self.photos) {
    [photo uploadForContentType:BVPhotoContentTypeComment
...
- (void)submitCommentWithPhotoUrls:(BVSubmissionAction)action
                         photoUrls:(nonnull NSArray<NSString *> *)photoUrls
                     photoCaptions:(nonnull NSArray<NSString *> *)photoCaptions
                           success:(nonnull CommentSubmissionCompletion)success
                           failure:
                               (nonnull ConversationsFailureHandler)failure {
  ...
  NSString *urlString =
      [NSString stringWithFormat:@"%@submitreviewcomment.json",
      ...
      dataTaskWithRequest:request
        completionHandler:^(NSData *data, NSURLResponse *response,
          ...
          @try {
            ...
            BVCommentSubmissionErrorResponse *errorResponse =
                [[BVCommentSubmissionErrorResponse alloc]
            ...
            if (httpError) {
              ...
              // Fire event now that we've confirmed the comment was
              // successfully uploaded.
              BVFeatureUsedEvent *commentQuestionEvent = [
              ...
              BVCommentSubmissionResponse *response = [
                  [BVCommentSubmissionResponse alloc] initWithApiResponse:json];

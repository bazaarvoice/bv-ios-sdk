...
+ (void)checkButtonsInSubviews:(NSArray*)subviews withTarget:(id)target withSelector:(SEL)targetSelector {
    ...
    for(UIView* subview in subviews) {
        ...
        [self checkButtonsInSubviews:subview.subviews withTarget:target withSelector:targetSelector];
    }
    ...
    for(UIGestureRecognizer* recognizer in gestureRecognizers) {
        if([recognizer isKindOfClass:[UIGestureRecognizer class]]){
            ...
            if([tapRecognizer cancelsTouchesInView]){
                ...
                NSAssert(false, @"UIGestureRecognizer must have `cancelsTouchesInView` set to false for the BVSDK to properly function.");

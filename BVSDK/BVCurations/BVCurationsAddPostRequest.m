...
- (id)initWithGroups:(NSArray<NSString *> *)groups
     withAuthorAlias:(NSString *)alias
...
- (id)initWithGroups:(NSArray<NSString *> *)groups
  ...
  self = [self initWithGroups:groups
              withAuthorAlias:alias
                    withToken:token
                     withText:text];
  if (self) {
    _image = image;
...
- (NSDictionary *)toDictionary {
  ...
  NSMutableDictionary *authorDict = [NSMutableDictionary
      dictionaryWithObjectsAndKeys:self.alias, @"alias", self.token, @"token",
                                   nil]; // author dict
  ...
  NSMutableDictionary *params = [NSMutableDictionary
      dictionaryWithObjectsAndKeys:self.groups, @"groups", authorDict,
                                   @"author", self.text, @"text", nil];
  ...
  if (self.unixTimeStamp > 0) {
    [params setObject:[NSNumber numberWithDouble:self.unixTimeStamp]
               forKey:@"timestamp"];
  }
  ...
  if (self.longitude != 0 && self.latitude != 0) {
    ...
    NSDictionary *coordinatesDict = [NSDictionary
        dictionaryWithObjectsAndKeys:latNum, @"x", longNum, @"y", nil];
    ...
    for (NSString *link in self.links) {
      [linksArray
          addObject:[NSDictionary
                        dictionaryWithObjectsAndKeys:link, @"url", nil]];
    }
  ...
  if (self.photos.count > 0) {
    ...
    for (NSString *photo in self.photos) {
      [photosArray
          addObject:[NSDictionary dictionaryWithObjectsAndKeys:photo,
                                                               @"remote_url",
                                                               nil]];
    }
...
- (NSString *)description {
  ...
  return [[NSString alloc] initWithData:self.serializeParameters

...
- (void)configureWithDictionary:(NSDictionary *)dict
    ...
    if (analyticsLocaleIdentifier) {
        analyticsLocale = [[NSLocale alloc]
            initWithLocaleIdentifier:analyticsLocaleIdentifier];
    } else {
        [[BVLogger sharedLogger]
            warning:[NSString
                        stringWithFormat:@"BVSDK is currently using user "
...
- (NSString *)urlRootShopperAdvertising {
    return self.configuration.staging
...
- (void)waitForProfileUpdatesByPollingWithStep:(NSTimeInterval)step
    ...
    while (currentStep > maxTimeInterval) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW,
                                     (int64_t)(currentStep * NSEC_PER_SEC)),

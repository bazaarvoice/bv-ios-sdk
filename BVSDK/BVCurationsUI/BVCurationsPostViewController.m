...
- (void)viewDidLoad {
    ...
    BVInViewEvent *inViewEvent = [[BVInViewEvent alloc]
           initWithProductId:@"none"
             ...
             withContainerId:@"CurationsSubmissionController"
...
- (void)setupNavBar {
    ...
    UIImage *bkgd =
        [self navBarBackgroundImage:_navBarColor logo:_logo size:size];
    [self.navigationController.navigationBar
        setBackgroundImage:bkgd
...
- (void)didSelectPost {
    ...
    BVCurationsPhotoUploader *uploader =
        [[BVCurationsPhotoUploader alloc] init];
    [uploader submitCurationsContentWithParams:_postRequest
        completionHandler:^{
...
- (void)completePost:(NSError *)error {
    if (!error) {
        BVFeatureUsedEvent *submittedPostEvent = [[BVFeatureUsedEvent alloc]
               initWithProductId:@"none"
                       withBrand:nil
                 withProductType:BVPixelProductTypeCurations
    ...
    [self dismissViewControllerAnimated:NO
                             completion:^{
                               if (self->_didCompletePost) {
                                   self->_didCompletePost(error);
                               }
                             }];
}

- (BOOL)isContentValid {
    return self.textView.text.length && _postRequest.alias.length &&
           _postRequest.token.length;
}
...
- (UIView *)loadPreviewView {
    ...
    imageView.image = [self proportionallyScaledImageOf:_postRequest.image
                                   inBoundingDimensions:imageView.frame.size];

    return imageView;
}

- (UIImage *)navBarBackgroundImage:(UIColor *)backgroundColor
                              logo:(UIImage *)logo
                              size:(CGSize)size {
    ...
    CGSize logoSize =
        [self proportionalSizeOf:logo.size
                  inBoundingSize:CGSizeMake(CGFLOAT_MAX, dimension)];
    CGRect logoFrame =
        CGRectMake(size.width / 2 - logoSize.width / 2, logoPadding / 2,
...
- (NSArray *)configurationItems {
    SLComposeSheetConfigurationItem *item =
        [SLComposeSheetConfigurationItem new];
    item.title = @"Username";
...
- (UIImage *)proportionallyScaledImageOf:(UIImage *)image
                    inBoundingDimensions:(CGSize)dimensions {
    CGSize imgSize = image.size;
    CGSize newSize =
        [self proportionalSizeOf:imgSize inBoundingSize:dimensions];
    ...
    CGContextRef context = CGBitmapContextCreate(
        nil, newSize.width, newSize.height, bpc, bpr, colorSpace, bitmapInfo);
    CGContextSetInterpolationQuality(context, kCGInterpolationHigh);

    CGContextDrawImage(context, CGRectMake(0, 0, newSize.width, newSize.height),
                       cgImage);
...
- (void)dismiss {
    [self dismissViewControllerAnimated:YES
                             completion:^{
                               if (self->_didPressCancel) {
                                   self->_didPressCancel();
                               }
                             }];
}

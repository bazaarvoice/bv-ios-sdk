...
- (void)testEnterOwnPlace {
  ...
  [[BVLocationManager sharedManager]
      callbackToDelegates:@selector(didBeginVisit:)
...
- (void)testEnterOtherPlace {
  ...
  NSDictionary *attributes =
      @{_clientIdKey : @"otherClient", @"type" : @"geofence"};
  [[BVLocationManager sharedManager]
      callbackToDelegates:@selector(didBeginVisit:)
           withAttributes:attributes];

  XCTAssert(!_delegateCalledBack,
            @"Delegate method should not have been performed");
}

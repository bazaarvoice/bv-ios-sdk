...
echo "    1) Verify that SDK version (in BVCore.h) equals Cocoapods version (in BVSDK.podspec)"
...
SDK_VER=$(grep "SDK_HEADER_VALUE @" Pod/BVCommon/BVCore.h) 
...
echo "Version appears to be $SDK_VER, is that correct (y/n)? (if not, check BVCore.h)"
read confirm
if [ $confirm != 'y' ]; then
    exit 1;
fi

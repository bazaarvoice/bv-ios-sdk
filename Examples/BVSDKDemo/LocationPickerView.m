...
- (void)layoutSubviews {
  ...
  if (!self.tableView) {
    ...
    if ([self.delegate respondsToSelector:@selector
      ...
      self.tableView.scrollIndicatorInsets =
          UIEdgeInsetsMake(64.0, 0.0, 0.0, 0.0);
    }
    ...
    if ([self.delegate
            respondsToSelector:@selector(locationPicker:tableViewDidLoad:)]) {
  ...
  if (!self.tableView.tableHeaderView) {
    CGRect tableHeaderViewFrame = CGRectMake(
        0.0, 0.0, self.tableView.frame.size.width, self.defaultMapHeight);
    UIView *tableHeaderView =
        [[UIView alloc] initWithFrame:tableHeaderViewFrame];
    tableHeaderView.backgroundColor = [UIColor clearColor];
    self.tableView.tableHeaderView = tableHeaderView;
  }

  if (!self.mapView) {
    self.defaultMapViewFrame =
        CGRectMake(0.0, -self.defaultMapHeight * self.parallaxScrollFactor * 2,
                   self.tableView.frame.size.width,
                   self.defaultMapHeight +
                       (self.defaultMapHeight * self.parallaxScrollFactor * 4));
    ...
    if ([self.delegate
            respondsToSelector:@selector(locationPicker:mapViewDidLoad:)]) {
  ...
  if (!self.mapTapGesture) {
    self.mapTapGesture = [[UITapGestureRecognizer alloc]
        initWithTarget:self
  ...
  if (!self.backgroundView) {
    UIView *view = [[UIView alloc]
        initWithFrame:CGRectMake(0.0, self.defaultMapHeight,
                                 self.tableView.frame.size.width,
...
- (void)setCustomCloseButton:(UIButton *)closeButton
  ...
  if (self.closeMapButton)
    [self.closeMapButton removeFromSuperview];
  self.closeMapButton = closeButton;
  self.closeButtonPoint = buttonPoint;
  [self.closeMapButton addTarget:self
                          action:@selector(didTapCloseMapViewButton:)
...
- (void)showCloseMapButton {
  if (!self.closeMapButton) {
    ...
    self.closeMapButton.frame = CGRectMake(self.closeButtonPoint.x,
                                           self.closeButtonPoint.y, 42.0, 42.0);
    [self.closeMapButton setImage:[UIImage imageForXIcon]
    ...
    [self.closeMapButton
        setFrame:CGRectMake(self.closeButtonPoint.x, self.closeButtonPoint.y,
                            self.closeMapButton.frame.size.width,
                            self.closeMapButton.frame.size.height)];
  }
...
- (void)expandMapView:(id)sender animated:(BOOL)animated {
  if ([self.delegate
          respondsToSelector:@selector(locationPicker:mapViewWillExpand:)]) {
  ...
  newMapFrame =
      CGRectMake(self.defaultMapViewFrame.origin.x,
                 self.defaultMapViewFrame.origin.y +
                     (self.defaultMapHeight * self.parallaxScrollFactor),
...
- (void)mapViewDidFinishExpanding {
  ...
  if ([self.delegate
          respondsToSelector:@selector(locationPicker:mapViewDidExpand:)]) {
...
- (void)hideMapView:(id)sender animated:(BOOL)animated {
  if ([self.delegate
          respondsToSelector:@selector(locationPicker:mapViewWillBeHidden:)]) {
  ...
  self.tableView.frame =
      CGRectMake(0, 480, tempFrame.size.width, tempFrame.size.height);
  [self insertSubview:self.mapView belowSubview:self.tableView];
...
- (void)mapViewDidFinishHiding {
  ...
  if ([self.delegate
          respondsToSelector:@selector(locationPicker:mapViewWasHidden:)]) {
...
- (void)centerMapOnUserLocationIfNecessary {
  if (self.shouldAutoCenterOnUserLocation && self.mapView.userTrackingMode) {
    MKCoordinateRegion centerCoordinate = MKCoordinateRegionMake(
        self.mapView.userLocation.coordinate, self.mapView.region.span);
    [self.mapView setRegion:centerCoordinate animated:YES];
  }
}
...
- (void)scrollViewDidScrollWithOffset:(CGFloat)scrollOffset {
  if ((self.isMapFullScreen == NO) && (self.isMapAnimating == NO)) {
    ...
    if (scrollOffset < 0) {
      ...
      // Pull to expand map?
      if (self.pullToExpandMapEnabled && (self.isMapAnimating == NO) &&
          (scrollOffset <= -self.amountToScrollToFullScreenMap)) {
        [self expandMapView:self];
      } else {
        mapFrameYAdjustment = self.defaultMapViewFrame.origin.y -
                              (scrollOffset * self.parallaxScrollFactor);
      }
    }

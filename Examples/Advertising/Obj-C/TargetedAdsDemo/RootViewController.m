...
- (void)viewDidLoad {
  ...
  UIBarButtonItem *adUnitTestBarButtonItem = [[UIBarButtonItem alloc]
      initWithImage:[UIImage imageNamed:@"gear.png"]
              style:UIBarButtonSystemItemCamera
             target:self
             action:@selector(adUnitTestButtonPressed)];
  ...
  [self.collectionView registerNib:[UINib nibWithNibName:@"BVAdTypesCell"
                                                  bundle:nil]
  ...
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(interstitialError)
                                               name:@"INTERSTITIAL_ERROR"
                                             object:nil];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  case 1:
    itemData = @{
      ...
      @"subtitle" : @"Interstitial ads fill up the entire screen on your "
                    @"customer's device"
    };
    break;
  case 2:
  ...
  BVAdTypesCell *cell =
      [collectionView dequeueReusableCellWithReuseIdentifier:@"cvCell"
  ...
  cell.backgroundImage.image =
      [UIImage imageNamed:[itemData objectForKey:@"imageName"]];
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  [self.navigationController.navigationBar
      setBarTintColor:[UIColor bazaarvoiceNavy]];

  // set "bazaarvoice:" logo in navigationBar
  UILabel *titleLabel =
      [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 100, 100)];
...
- (void)adUnitTestButtonPressed {
  ...
  GeneralizedDemoViewController *demoViewController =
      [[GeneralizedDemoViewController alloc] init];
...
- (void)requestInterstitial {
  ...
  self.interstitialDemo =
      [[InterstitialDemo alloc] initWithRootViewController:self];
  [self.interstitialDemo requestInterstitial];
}

- (void)presentContentNativeScreen {
  NativeContentAdDemoViewController *viewController =
      [[NativeContentAdDemoViewController alloc]
          initWithNibName:@"NativeContentAdDemoViewController"
                   bundle:nil];
  [self presentViewController:viewController animated:YES completion:nil];
}

- (void)presentBannerScreen {
  BannerDemoViewController *viewController = [[BannerDemoViewController alloc]
      initWithNibName:@"BannerDemoViewController"
               bundle:nil];
  [self presentViewController:viewController animated:YES completion:nil];
}

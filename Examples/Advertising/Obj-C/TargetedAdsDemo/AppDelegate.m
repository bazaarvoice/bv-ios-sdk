...
#warning Add your own bvsdk_config files to project
  ...
  // See the BVSDK wiki for more discussion on how to create this user auth
  // string:
  // https://github.com/bazaarvoice/bv-ios-sdk/wiki/BVSDK-UserAuthentication A
  // user auth string would contain data in a query string format. For example:
  //     userid=Example&email=user@example.com&age=28&gender=female&facebookId=123abc

  // Example auth string used, pre-populated with a small profile interested in
  // "pets",
  ...
  NSString *myClientId =
      [[[BVSDKManager sharedManager] configuration] clientId];
  NSString *myShopperAdvertisingKey =
      [[[BVSDKManager sharedManager] configuration] apiKeyShopperAdvertising];
  if ([myClientId isEqualToString:@"REPLACE_ME"] ||
      [myShopperAdvertisingKey isEqualToString:@"REPLACE_ME"]) {
...
- (void)setupWindow {
  ...
  [[UIApplication sharedApplication]
      setStatusBarStyle:UIStatusBarStyleLightContent
               animated:NO]; // set status bar color
...
- (void)showNotConfiguredError {
  ...
  NSString *alertMessage = @"Make sure you have set your API Key and Client ID "
                           @"in project files bvsdk_config_prod.json and "
                           @"bvsdk_config_staging.json . Please contact "
                           @"Bazaarvoice if you do not have an API key.";

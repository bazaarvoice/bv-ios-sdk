...
- (void)viewDidLoad {
    ...
    self.adTypeControl = [[UISegmentedControl alloc] initWithItems:@[@"Interstitial", @"Banner", @"Native" ]];
    self.adTypeControl.frame = CGRectMake(20, 20, self.view.bounds.size.width - 40, 30);
    [self.adTypeControl addTarget:self action:@selector(adTypeSwitched:) forControlEvents:UIControlEventValueChanged];
    [self.view addSubview:self.adTypeControl];
    
    self.adTypeControl.selectedSegmentIndex = 0;
    
    
    sharedControlsOriginalFrame = CGRectMake(0, CGRectGetMaxY(self.adTypeControl.frame), self.view.bounds.size.width, 200);
    self.sharedControlsContainerView = [[UIView alloc] initWithFrame:sharedControlsOriginalFrame];
    
    self.bannerSizeWidth = [[UITextField alloc] initWithFrame:CGRectMake(20,
                                                                        CGRectGetMaxY(self.adTypeControl.frame) + 20,
    ...
    self.nativeAdTypeControl = [[UISegmentedControl alloc] initWithItems:@[@"Content", @"Custom"]];
    self.nativeAdTypeControl.frame = nativeAdTypeControlFrame;
    self.nativeAdTypeControl.selectedSegmentIndex = 0;
    [self.nativeAdTypeControl addTarget:self action:@selector(nativeAdTypeSwitch:) forControlEvents:UIControlEventValueChanged];
    self.nativeAdTypeCustomId = [[UITextField alloc] initWithFrame:CGRectMake(20,
                                                                              CGRectGetMaxY(nativeAdTypeControlFrame) + 10,
    ...
    CGRect adUnitIdFrame = CGRectMake(20,
                                      20,
                                      self.view.bounds.size.width - 40,
                                      40);
    ...
    CGRect keyValFrame = CGRectMake(20,
                                    CGRectGetMaxY(self.adUnitIdField.frame) + 10,
                                    self.view.bounds.size.width/2 - 40,
                                    40);
    ...
    self.goButton.frame = CGRectMake(20,
                                                               CGRectGetMaxY(self.key2Field.frame) + 20,
                                                               self.view.bounds.size.width - 40,
                                                               40);
    ...
    [self.goButton setTitleColor:[UIColor grayColor] forState:UIControlStateSelected];
    ...
    self.goButton.backgroundColor = [UIColor colorWithRed:1.0 green:1.0 blue:0.99 alpha:1.0];
...
-(void)setSelected:(bool)selected button:(UIButton*)button {
    button.backgroundColor = selected ? [UIColor colorWithRed:102.0/255.0 green:211.0/255.0 blue:3.0/255.0 alpha:1.0] : [UIColor whiteColor];
...
-(void)goPressed {
    ...
    if(self.adTypeControl.selectedSegmentIndex == 0) {
        ...
        [self requestNativeAd:adUnitId targeting:targeting customTemplate:isCustomTemlate];
    }
}
...
-(void)requestBanner:(NSString*)adUnitId targeting:(NSDictionary*)customTargeting size:(CGSize)bannerSize {
    if(self.bannerView){
        [self.bannerView removeFromSuperview];
    }
    
    GADAdSize size = [self getAdSize:bannerSize];
    if(size.size.width == kGADAdSizeInvalid.size.width && size.size.height == kGADAdSizeInvalid.size.height){
        return;
    }
    ...
    self.bannerView.backgroundColor = [UIColor colorWithRed:0.93 green:0.93 blue:0.93 alpha:1.0];
    self.bannerView.frame = CGRectMake(0,
                                       self.view.bounds.size.height - self.bannerView.bounds.size.height,
...
-(GADAdSize) getAdSize:(CGSize)size {
    ...
    [[[UIAlertView alloc] initWithTitle:@"Invalid banner size" message:@"invalid banner ad size (for this demo app)." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil] show];
...
-(void)requestNativeAd:(NSString*)adUnitId targeting:(NSDictionary*)customTargeting customTemplate:(bool)custom{
...
- (void)adLoader:(GADAdLoader *)adLoader didReceiveNativeContentAd:(GADNativeContentAd *)nativeContentAd {
    ...
    UIView* overlay = [[UIView alloc] initWithFrame:self.nativeResult.view.bounds];
    ...
    [closeButton addTarget:self action:@selector(nativeCloseButtonPressed) forControlEvents:UIControlEventTouchUpInside];
...
- (void)adLoader:(GADAdLoader *)adLoader didReceiveNativeCustomTemplateAd:(GADNativeCustomTemplateAd *)nativeCustomTemplateAd {
    ...
    for(NSString* assetKey in nativeCustomTemplateAd.availableAssetKeys) {
        ...
        if(str){
            UILabel* label = [[UILabel alloc] initWithFrame:CGRectMake(0, labelY, labelWidth, labelHeight)];
            label.text = str;

...
class NativeContentAdDemoViewController: UIViewController, GADNativeContentAdLoaderDelegate {
    ...
    func adLoader(_ adLoader: GADAdLoader!, didReceive nativeContentAd: GADNativeContentAd!) {
        ...
        let contentAdView = Bundle.main.loadNibNamed("NativeContentAdView", owner: nil, options: nil)![0] as! GADNativeContentAdView
        ...
        (contentAdView.callToActionView as! UIButton).setTitle(callToActionText, for: UIControlState())
        ...
        contentAdView.frame = CGRect(x: padding,
                                    y: self.view.bounds.size.height * 0.18,
                                    width: self.view.bounds.size.width - padding*2,
                                    height: self.view.bounds.size.height * 0.4)
        ...
        contentAdView.layer.borderColor = UIColor.green.cgColor
        contentAdView.layer.borderWidth = 0.5;
        contentAdView.layer.cornerRadius = 2;
        contentAdView.layer.shadowColor = UIColor.gray.cgColor
        contentAdView.layer.shadowOffset = CGSize(width: 0,height: 5);

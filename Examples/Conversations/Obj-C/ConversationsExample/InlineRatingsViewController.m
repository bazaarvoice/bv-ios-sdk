...
- (void)viewDidLoad {
    ...
    [self.inlineReviewsTableView registerNib:[UINib nibWithNibName:@"StatisticTableViewCell" bundle:nil] forCellReuseIdentifier:@"StatisticTableViewCell"];
    
    NSArray *productIds = @[@"test1", @"test2",@"test3", @"test4", @"test5", @"test6"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.statTypeLabel.text = [NSString stringWithFormat:@"Product ID: %@", [self.productStatistics objectAtIndex:indexPath.row].productId];
    long totalReivewCount = [[self.productStatistics objectAtIndex:indexPath.row].reviewStatistics.totalReviewCount longValue];
    long averageOverallRating = [[self.productStatistics objectAtIndex:indexPath.row].reviewStatistics.averageOverallRating longValue];
    long overallRatingRange = [[self.productStatistics objectAtIndex:indexPath.row].reviewStatistics.overallRatingRange longValue];
    
    cell.statValueLabel.text = [NSString stringWithFormat:@"Total Reivew Count: %ld\nAverage Overall Rating: %ld\nOverall Rating Range: %ld", totalReivewCount,averageOverallRating,overallRatingRange];

...
- (void)viewDidLoad {
    ...
    [self.authorProfileTableView registerNib:[UINib nibWithNibName:@"StatisticTableViewCell" bundle:nil] forCellReuseIdentifier:@"StatisticTableViewCell"];
    [self.authorProfileTableView registerNib:[UINib nibWithNibName:@"MyReviewTableViewCell" bundle:nil] forCellReuseIdentifier:@"MyReviewTableViewCell"];
    [self.authorProfileTableView registerNib:[UINib nibWithNibName:@"MyQuestionTableViewCell" bundle:nil] forCellReuseIdentifier:@"MyQuestionTableViewCell"];
    [self.authorProfileTableView registerNib:[UINib nibWithNibName:@"MyAnswerTableViewCell" bundle:nil] forCellReuseIdentifier:@"MyAnswerTableViewCell"];
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    if (!self.authorResponse) return 0;
    
    BVAuthor *author = self.authorResponse.results.firstObject;
    
    switch (section) {
        case ProfileStats:
            return [self.authorResponse.totalResults integerValue]; // should always be 1
            break;
        case IncludedReviews:
...
- (NSString *)createAuthorStatsString:(BVAuthor *)author {
    ...
    if (!author) return @"No author stats.";

    NSString *summaryText = [NSString stringWithFormat:@"Stats for: %@\n", author.userNickname];
    
    if (author.userLocation){
        summaryText = [summaryText stringByAppendingFormat:@"Location: %@\n", author.userLocation];
    }
    
    if (author.reviewStatistics){
        summaryText = [summaryText stringByAppendingFormat:@"Reviews (%@)\n", author.reviewStatistics.totalReviewCount];
    }
    
    if (author.qaStatistics){
        summaryText = [summaryText stringByAppendingFormat:@"Questions (%@)\n", author.qaStatistics.totalQuestionCount];
        summaryText = [summaryText stringByAppendingFormat:@"Answers (%@)\n", author.qaStatistics.totalAnswerCount];
    }
    
    if (author.contextDataValues){
        summaryText = [summaryText stringByAppendingFormat:@"Context Data Values (%lu) ", (unsigned long)author.contextDataValues.count];
        
        for (BVContextDataValue *contextData in author.contextDataValues){
            summaryText = [summaryText stringByAppendingFormat:@"[%@:%@]", contextData.identifier, contextData.value];
        }
    ...
    if (author.badges){
        summaryText = [summaryText stringByAppendingFormat:@"Badges (%lu) ", (unsigned long)author.badges.count];
        
        for (BVBadge *badge in author.badges){
            summaryText = [summaryText stringByAppendingFormat:@"[%@:%@]", badge.identifier, badge.contentType];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (indexPath.section) {
        ...
        case ProfileStats:
        {
            StatisticTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"StatisticTableViewCell"];
        ...
        case IncludedQuestions:
        {
            MyQuestionTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MyQuestionTableViewCell"];

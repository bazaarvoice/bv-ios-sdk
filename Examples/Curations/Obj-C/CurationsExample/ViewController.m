...
- (void)viewDidLoad {
  ...
  UIBarButtonItem *submitButton =
      [[UIBarButtonItem alloc] initWithTitle:@"Add Photo"
...
- (void)didTapAddPhotoButton {
  ...
  // Here we load our request with the groups we want to subit to and additional
  // info.
  BVCurationsAddPostRequest *request =
      [[BVCurationsAddPostRequest alloc] initWithGroups:@[]
                                        withAuthorAlias:@"authorAlias"
                                              withToken:@"token"
                                               withText:@"Hello"];

  // We've hard-coded an image here for testing, where you would normally have
  // the user select from a gallery or camera.
  request.image = [UIImage imageNamed:@"curations_test_image"];

  // Now we just post the share view controller with a couple of styling
  // options.

  BVCurationsPostViewController *shareVC =
      [[BVCurationsPostViewController alloc]
  ...
  [self.navigationController presentViewController:shareVC
                                          animated:YES
                                        completion:^{
                                            // completion
                                        }];

...
- (void)submitRating
{
    ...
    if(self.imageUrl)
    {
        ...
        mySubmission.productId = @"10sdfsdf000132";
        ...
        mySubmission.rating = rating;
        mySubmission.title = self.titleTextField.text;
        mySubmission.reviewText = self.reviewTextView.text;
        mySubmission.userNickname = self.nicknameTextField.text;
        
        // This type name is the index of the phot in the upload... it maps to photo_1 = ...
        [mySubmission addPhotoUrl:self.imageUrl withCaption:nil];
...
-(void) didReceiveResponse:(NSDictionary *)response forRequest:(id)request
{
    ...
    if([request isKindOfClass:[BVMediaPost class]])
    {
        ...
        // new photo url so that it can be included with the form submission
         NSString *url = [[[[response objectForKey:@"Photo"] objectForKey:@"Sizes"] objectForKey:@"normal"] objectForKey:@"Url"];
    ...
    else if([request isKindOfClass:[BVPost class]])
    {
        ...
        if([self hasErrors:response]) {
            ...
            // can display it.
            confirmController.confirmData = response;
